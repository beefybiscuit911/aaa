...
//  2019-04-30: DirectX11: Added support for special
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != NULL) {
        ...
        // the user to request the renderer to reset render state.) if
        // (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
        //    ImGui_ImplDX11_SetupRenderState(draw_data, ctx);
        // else
...
bool ImGui_ImplDX11_CreateDeviceObjects() {
  ...
  // Create the vertex shader
  {
    ...
    if (g_pVertexShaderBlob ==
        NULL) // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to get error
              // showing in (const char*)pErrorBlob->GetBufferPointer(). Make
              // sure to Release() the blob!
      return false;
    if (g_pd3dDevice->CreateVertexShader(
            (DWORD *)g_pVertexShaderBlob->GetBufferPointer(),
  ...
  // Create the pixel shader
  {
    ...
    if (g_pd3dDevice->CreatePixelShader(
            (DWORD *)g_pPixelShaderBlob->GetBufferPointer(),
